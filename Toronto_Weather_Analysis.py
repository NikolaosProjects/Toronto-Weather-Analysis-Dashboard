...
app.layout = html.Div(style={'text-align': 'Center', 'backgroundColor':colors['background'], 'color': colors['text']}, children =
                      ...
                      #github link
                      [html.Div(style = {'text-align': 'Left', 'font-size': '13px'}, children = [html.H6("GitHub:", style = {'display': 'inline-block', 'font-size': '13px'}),
                       html.A("https://github.com/NikolaosProjects/Toronto-Weather-Analysis-Dashboard", href="https://github.com/NikolaosProjects/Toronto-Weather-Analysis-Dashboard", target="https://github.com/NikolaosProjects/Toronto-Weather-Analysis-Dashboard")]),
                          
                       #title
                       html.Div(children = [html.H1("Toronto Weather Analysis"),
                       ...
                       #space between info text and warnings
                       html.Br(),
                       html.Br(),
                       ...
                       #breaks between warnings and selection boxes
                       html.Br(),
                       html.Br(),
                      ...
                      dbc.Row(
                          ...
                          [dbc.Col(html.Div(children = [html.H5('Please Select Year:', style={"margin-left": "40px", 'color': colors['text']})]), width = {'order': 1, 'size': 2, 'offset': 1}),
                           ...
                           dbc.Col([dcc.Dropdown(id="year_selection", options= [{"label": "2017", "value": 2017},
                                                                       ...
                                                                       {"label": "2020", "value": 2020}], multi=False, value=2018, style = {'color' : colors['background']}),
...
def update_graph(year, month, selected_rows):
    ...
    fig.add_trace(go.Scatter(x = data["Date"][max_ind], y = data[graph_selection][max_ind], mode="markers+text", name="Maximum = " + (data[graph_selection][max_ind]).to_string()[-6:] + symbol, text=[(data[graph_selection][max_ind]).to_string()[-6:] + symbol], textposition="top right", marker = dict(size = 10), textfont_color= 'Red'))
    ...
    fig.add_trace(go.Scatter(x = data["Date"][min_ind], y = data[graph_selection][min_ind], mode="markers+text", name= "Minimum = " + (data[graph_selection][min_ind]).to_string()[-6:] + symbol, text=[(data[graph_selection][min_ind]).to_string()[-6:] + symbol], textposition="bottom right", marker = dict(size = 10), textfont_color= 'Green'))
    ...
    fig.update_layout({"plot_bgcolor": colors['background'], 'paper_bgcolor': "rgba(0,0,0,0)"},
                      ...
                      yaxis=dict(title='', showgrid=False, zeroline = False, tickmode = 'linear', dtick = ticks),
    ...
    #removing the zoom in the website for the time series graph
    ...
    #disable plot zooming in the website for the weather conditions distribution barchart
