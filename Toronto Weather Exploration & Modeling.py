...
app.layout = html.Div(style={'text-align': 'Center', 'backgroundColor':colors['background'], 'color': colors['text']}, children =
                      ...
                      dbc.Row(
                          ...
                          #1st row begins here
                          [dbc.Col(html.Div(children = [html.H5('Please Select Year:', style={"margin-left": "110px", 'color': colors['text']})]), width = {'order': 1, 'size': 2, 'offset': 1}),
...
def update_graph(year, month, selected_rows):
    ...
    if month == 20:    
        ...
        fig.update_xaxes(nticks = 31, tickangle=45)
    ...
    for x, y in zip(conditions, frequency):
        fig1.add_layout_image(
            dict(
                source = Image.open("C:\\Users\\nick-\\OneDrive\\Desktop\\Weather Project\\Bargraph Icons\\" + x + ".png"),
                x = x,
                y = y+28,
                xref = "x",
                yref = "y",
